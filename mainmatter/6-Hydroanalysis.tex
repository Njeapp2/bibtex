...
In the expression above, $a$ is a constant and $b$ an index exponent. \textcite{schmidtStageDischargeRelationshipOpen2011} note several limitations related to these rating curves. For example, discharge measurements typically scatter and therefore do not show a unique relation with the stage. Also, the underlying physics of the open channel are not captured in the rating. Nevertheless, the relation as given in Equation \ref{eq:powerlaw} is used to approximate dependencies between the variables. 
...
$ F_{w} = 0.7664 * 1.3 = 0.9963  \, \text{kN/m}$ per day. 
...
$$
This also occurs for a short period of time, but 6 times a day. Hence, it is not a static load and needs to be considered at different time intervals. The same approach as for the wave load is used, adding the dynamic load factor of 1.3 to the previously calculated value. 
$$
F_c = 1.350 * 1.3 = ~\text{[kN/m]}
$$
...
$$
F = F_w + F_c = 1.350 \cdot 6 + 0.9963 = 54.08 ~\text{[kN/m]}
$$
