...
\subsubsection{Estimated suspended load}
...
Gebruik Engelund-Hansen met relevante aannames, op basis van velocity time series 
...
The Acoustic Doppler Current Profiler (ADCP) measures the flow velocity and depth across a river cross section by dividing the measurements into vertical bins for every ensemble. This data can be used to create a flow velocity field for the whole cross section and velocity profiles for multiple locations along the cross section. These velocity profiles are used to determine the Law of the Wall fit, see formula \ref{eq:law_of_the_wall}.
...
The reference depth ($a$) and the near-bed reference concentration ($C_a$) were based on the lowest measured suspended sediment concentrations for each cross section. By inspecting the Rouse profile at the ensemble where the sediment measurements were taken, the settling velocity ($w_s$) was determined, as no information on suspended sediment size was available. 
