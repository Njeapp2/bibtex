...
This leaves us with the theory of waves \autocite{week 2.2} which states that there are two types of waves you can get energy from:
There are two types of waves for which we can calculate energy.

\begin{itemize}
    \item periodic waves (defined as waves which are generated in a specific periodic pattern, e.g. 1 wave each 10 seconds)
    \item random waves (waves that generated randomly)
\end{itemize}
...
\[
\boxed{E_{pw} = \left(\frac{1}{8}\right) \cdot \rho \cdot g \cdot H_{m0}^2}
\]

\[
\boxed{E_{rw} = \left(\frac{1}{16}\right) \cdot \rho \cdot g \cdot H_{m0}^2}
\]

\noindent where:
\begin{itemize}
    \item \(p\) is the density of river water: 1000 [kg/m\textsuperscript{3}];
...
$
E_{pw} = \left(\frac{1}{8}\right) \cdot 1000 \cdot 9.81 \cdot (0.25)^2
$
$
E_{pw} = 76.64 \, \text{J/m}^2 = 0.7664 \, \text{kN/m}
$

For every wave coming from the boat, in reality this load has to be times 10 for every time a boat passes by.
