...
The result of this time series is given in Figure \ref{fig:timeseries fine sediments}. It is important to note that this relation for fine sediment concentrations is considered useful for the Paraná Guazú as well (\cite{reMETODOLOGIAPARAGENERACION2009}). 
...
The Acoustic Doppler Current Profiler (ADCP) measures the flow velocity and depth across a river cross section by dividing the measurements into vertical bins for every ensemble. This data can be used to create a flow velocity field for the whole cross section, see Appendix \ref{appendix:fieldwork results}, and velocity profiles for multiple locations along the cross section. These velocity profiles are used to determine the Law of the Wall fit, see formula \ref{eq:law_of_the_wall}.
...
The total suspended sediment flux per cross section is calculated by 

\begin{table}[H]
    \centering
    \caption{Flow rates at different cross sections (kg/s)}
    \label{Calculated suspended sediment flux}
    \begin{tabular}{c c c}
        \hline
