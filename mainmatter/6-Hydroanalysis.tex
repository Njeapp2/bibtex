...
The importance of the Paraná Guazú has been identified. In view of the study area, the discharge of Paraná Guazú used above, sums the contribution of the Ibicuy and partly of the Paraná. At Puerto Ibicuy, no active discharge measurements are made. \textcite{brokChannelNetworkMorphodynamics2022}mention a river discharge of approximately 
1600 m$^3$/s, rendering the contribution of the Ibicuy branch relatively small. Therefore, further analysis is performed on the Brazo Largo station downstream. Here, the first step is to look at water elevations and how these relate to discharge.

Traditionally, stage-discharge relations are described by rating curves that describe power-law dependencies between variables. This can be written in the following form, with $Q$ the discharge and $h$ the water level or stage:
...
Figure \ref{fig:Measured suspended sediment concentrations} shows, that for all three cross sections, the suspended sediment concentration is the highest near the bed, which is typical for suspended sediment. The steeper profile for cross section 2 indicates higher turbulence or finer particles compared to cross sections 1 and 3. Higher turbulence combined with finer particles causes sediment to stay suspended more easily, which results in a steeper concentration gradient with depth.
...
The river banks are the separation between the river and the land. Therefore, they are an important element to consider when evaluating the hydrodynamic changes of the river due to sand mining. Starting from the wave theory, one create the most realistic approach to the analysed situation, and calculate the loads on the river banks. Furthermore, one can evaluate what the effect is of the water level on the river banks, their correlation with bank erosion and satellite data. 

\subsection{Damage on the river bank}
\label{chap 6: effect and damage on the river bank}
In addition to the study on the effects of sand mining on the river bed, it is as important to look at the effects of water on the river banks. Generally speaking, there are several ways how the water could have a negative impact on the river bank. The river bank failure may be caused by house placement, water saturation, weight on the river bank, vegetation, and/or tectonic activity, but most of them due to the rise of the water level. The reason behind this is that the river bank is naturally eroded when the water exerts a force on it \autocite{governmentofsouthaustraliaRiverbankCollapse2024}.
...
Consequently, after some time the upper overbank area has less and less support underneath, thus becoming unstable and partly falling down. 
...
Relevant to the previous subsection, the situations that contribute to this problem should be investigated. These are the elevation of the water level due to the high astronomical tide, and the elevation of the water level due to the waves.

The gravitational forces of the astronomical tide yield a change in water level, and is also responsible for tidal waves. These are induced by the gravitational pull, in the direction of the moving current. The tidal forcing has been treated in Section \ref{sec: tidal forcing}. Consequently, these do not contribute enough to the water elevation and wave load impact to be considered in this section.

The other types of waves that can be investigated for this study are wind waves and ship induced waves. Wind waves are produced with help of a striking distance called the fetch, which happens when the wind blows on the surface of the water, gathering energy which translates into a wave. In order to find the significant wave height of this situation, one takes a sample of wave heights from buoys and takes the average of the highest third waves \autocite{arrigaLecture2CIEM40002025}.But, given that the river is not straight, the fetch or striking distance in the Parana Guazu is overall quite short, as seen in Figure \ref{Figure 1.1}. This means that the impact of the wind waves can theoretically only be quite small. Therefore, this impact will not be taken into consideration for the calculation. 
...
Following the information about tides, waves and currents one might ask how the ship induced waves may exert a force on the river bed. This can be answered with the help of wave theory. The first relevant parameters to take into account are the ship waves in rivers. These are assumed to be between 0.25 and 0.75 meters tall according to Table \ref{tab:standard_values}. Based on observations during the field trip, the value of 0.25 meter wave height is chosen as the cargo ships do not travel with a speed significant enough to reach a height of 0.75 m at the riverbed 100 meters away.
...
Since there are 6 boats passing by every day, and each boat creates approximately 10 ship induced waves, a dynamic loading system would be required to solve this situation. Due to a lack of information and resources this dynamic loading system is not an option. Therefore, it is chosen to choose for a slight simplification of the model by transforming it into a static system. 

Summing all the forces for every wave passing by would be incorrect. In order to successfully choose an approach to the static model the method of cyclic loading, assuming a dynamic load factor, was chosen. According to dynamic wave loading, a factor of 1.2 to 1.5 can be used to determine the cyclic loading of a wave\autocite{Cuomo et all}. This factor depends on the type of structure and soil , is strongest on a concrete, but for a bank made out of clay/sand the value of the factor sits at 1.3. The final value of the ship induced wave load is therefore:

$ F_{w} = 0.766 * 1.3 = 0.996  \, \text{kN/m}$ per day. 
...
$$
This also occurs for a short period of time, but 6 times a day. It is also a dynamic system and should be considered at different time intervals. Hence, the same approach as for the wave load is used, adding the dynamic load factor of 1.3 to the previously calculated value. 
$$
F_c = 1.350 * 1.3 = 1.755 ~\text{[kN/m]}
$$
Therefore the total load effect of the ship waves and current on the bank in a static system would be:
