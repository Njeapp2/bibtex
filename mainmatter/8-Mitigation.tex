...
The design of the sheet pile wall is based on the cantilever sheet pile wall. As briefly touched on in Section \ref{section:sheet_pile_wall}, the cantilever sheet pile is used for retaining heights up to 5 meters and gets its support from the soil and hydrostatic pressure. In Figure \ref{fig:problem_description_sheetpiles}, a sketch of the situation, made in Python, is provided. The height that needs to be retained, $4.0 \ m$, is indicated with $Z$. The embedment depth of the cantilever sheet pile, which will need to be determined iteratively, is noted as $t$. In this sketch, the groundwater table, $-1.0 \ m$, is indicated by the dashed line and the abbreviation $GWT$. The water level of the river is indicated as $WL$, which, in this situation, is $-3.0 \ m$ below the ground surface. This will produce the most unfavorable situation and give the largest embedding length for the sheet pile.
...
\subsubsection{Blum-sheet pile wall}
...
As derived from the borehole and the overview in Table \ref{tab:soil_layers}, the soil profile with the specific layers is added to the cross-section of the problem. In Figure \ref{fig:soil_profile_cross_section} the geotechnical properties of each layer is shown. 
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.60\linewidth]{figures/ch8/effective_stress.png}
...
The balance of moments around the lowest part of the sheet pile should be zero,  stated in Equation \ref{eq:moment}. In Figure \ref{fig:final_moments_balance}, this point, "O", is indicated. Additionally, the forces acting due to both the earth and hydrostatic pressure are shown. From the acting forces and the distance to point "O", the moments are computed. 
...
In Table XX, it can be seen that the balance of moments is not equal to zero. Indicating an iterative calculation, increasing the embedding depth, and checking at which depth the moments are equal to zero, is required. After the iteration process, in Python, the embedding depth is set to be $9.713$ meters. In Table \ref{tab:forces_arms_moments_9713}, the values of the forces and moments are shown, and the resulting moment around point "O" is close to zero, $0.038 \ kNm/m$. As for the safety factor, the Blum method says the found embedding depth should be increased by $20\%$, giving the following depth:
...
A cantilever sheet pile wall should be designed based on two conditions. These conditions are the Ultimate Limit State (ULS) and the Serviceability Limit State (SLS). In the ULS the, worst possible forces that can occur during the the span of the sheet pile have to be analyzed concerning the safety of people and structure. In the SLS checking the structure under normal conditions where the function and appearance of the structure is concerned.

For the design, this means that safety factors for the ULS and SLS should be included, regarding to Eurocode 7. For the analysis, multiple combinations should be verified, and the most unfavorable is used for the design. Two combinations, within design approach 1, stated in Eurocode 7, are evaluated in Appendix XX. However, for the calculation in this section, the most unfavorable combination is taken and elaborated.

The most unfavorable combination is a low mean water level, and safety factors of design approach 1 (DA1-2). The safety factors are shown in Table \ref{tab:partial_factors}, and the partial factors on loads, materials and actions to safeguard the sheet pile in ULS are shown. In this calculation, the hydrostatic loads and the earth pressures are permanent. For the SLS, all the safety factors are set at 1.0. The combination and safety factors of Table XX result in the final embedding depth in the ULS, $t = 10.196$ meters,  shown in Figure \ref{fig:uls_moments_balance_forces}. From Table XX it can be seen that the balance of moments at this depth is close to zero, $-0.076 \ kNm/m$.

% WATER PERMANENT VERANDEREN IN BEREKENING.

\begin{equation}
    t_{final} = 1.20 \cdot 10.196 = 12.2 \ m
\end{equation}
...
For the verification of bending, it is important that the design bending moment, $M_{Ed}$, is smaller than the bending resistance, $M_{Rd}$, of the profile. The maximum bending moment, $M_{Ed}$, acting at the point of zero shear is $699.8 \ kNm/m$, shown in Figure XX. Within Class 2, Equation \ref{eq:bending_plastic} is used to calculate the bending resistance.
...
\begin{equation}
    \frac{M_{Ed}}{M_{Rd}} = \frac{699.8}{997.6} = 0.70 
\end{equation}
...
For the verification of shear, it is important that the design shear force, $V_{Ed}$ is smaller than the plastic shear resistance, $V_{pl,Rd}$, of the profile. The maximum shear force, $V_{Ed}$, is $305.1 \ kN/m$, shown in Figure XX. Equation \ref{eq:shear_verification} is used to calculate the shear force resistance.
...
\begin{equation}
    \frac{V_{Ed}}{V_{pl,Rd}} = \frac{305.1}{1468.4} = 0.21 
\end{equation}
