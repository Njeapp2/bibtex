...
The secondary flow coefficient used in the base model is 0.5. The influence of this parameter is tested by simulating with both a higher coefficient (1.0) and a lower coefficient (0.1). By inspecting the depth-averaged flow velocity profile in different cross sections of interest, it was found that the shape and magnitude of the profile remained almost identical. For the horizontal eddy viscosity, both 1.0 m\textsuperscript{2}/s and 5.0 m\textsuperscript{2}/s were tested. These values are determined using the Péclet number. 
